...
function validateField(field, value) {
  ...
  if ((field === "cell" || field === "home") && value) {
  ...
  if (field === "zip" && value && !/^\d{5}$/.test(value)) return "ZIP must be 5 digits."
...
export default function OwnerForm({ next, back, formData, setFormData }) {
  ...
  if (!o.dob) handleChange("dob", "1975-01-01")
  ...
  const isFormValid = () => {
    for (const field of REQUIRED) {
      if (validateField(field, o[field])) return false
    }
    return true
  }
  ...
  const handleNext = () => {
    ...
    next(!isNaN(pct) && pct < 51 ? "partner" : "signature")
  }

  const titleOptions = ["Owner","Founder","Co-Founder","Managing Partner","Principal","Managing Member","Partner","Chief Executive Officer (CEO)","President"]
  const ficoOptions  = ["700 +","650 – 699","600 – 649","550 – 599","525 – 549","500 – 524","475 – 499","Sub 474"]
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <FloatSelect label="FICO Score" field="fico" value={o.fico} onChange={handleChange} options={ficoOptions} required={R} submitAttempted={submitAttempted} />

        <FloatInput label="Home Address" field="address" value={o.address} onChange={handleChange} required={R} submitAttempted={submitAttempted} />
        <FloatInput label="City"         field="city"    value={o.city}    onChange={handleChange} required={R} submitAttempted={submitAttempted} />

        <FloatSelect label="State" field="state" value={o.state} onChange={handleChange} options={states} required={R} submitAttempted={submitAttempted} />
        <FloatInput  label="ZIP"   field="zip"   value={o.zip}   onChange={handleChange} required={R} submitAttempted={submitAttempted} />
